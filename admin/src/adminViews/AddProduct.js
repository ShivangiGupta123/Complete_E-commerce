...
//   const [pic,setPic] = useState({myFile:''});
  ...
  const imageuploader = (e) => {
    ...
    setPost({...post,profilePic:e.target.files[0]});
...
//   console.log("pic is>>",pic)
  ...
  const submit = (e) => {
    ...
    formData.append("myFile",  post.profilePic , post.profilePic.name);
  ...
  return (
    <div class="login-dark test col">
      <form
        style={{ backgroundColor: "slategrey", marginLeft: 23 }}
        onSubmit={submit}
      >
        ...
        <div className="form-group">
        <select className=" form-group custom-select" name='categoryId'  value={post.categoryId} onChange={(e) => changevalue(e)}  
        ...
        {categories.map((item, key) => {
          ...
          return (
            ...
            <option value={item._id} key={key} onChange={(e) => changevalue(e)}>{item.categoryName}</option>
          )
